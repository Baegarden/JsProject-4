...
titleStack.backgroundColor = new Color("#FFFFFF", 1)
...
for (const item of res.groups[0].ranking){
    ...
    if(Math.abs(ranking - myTeamRanking) <= 2){
        ...
        if (ranking <= 3)
            clStack.backgroundColor = new Color("#FFFFFF", 1)
        // 강등권 팀 설정
        else if (ranking >= 11)
            clStack.backgroundColor = new Color("#FFFFFF", 1)
        ...
        // MyTeam 설정
        if(item.team.idInternal == myTeamId) 
            teamStack.backgroundColor = new Color("#FFFFFF", 0.2)
        const underStack = widget.addStack()
        underStack.size = new Size(151, 1)
        underStack.borderColor = new Color("#FFFFFF", 0.5)
...
function createStack(stack, text, width, isTitle) {
    ...
    if (isTitle == 1){
        elementText.textColor = new Color("#000000", 1)
