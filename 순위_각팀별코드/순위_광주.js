...
widget.backgroundColor = new Color("#FFD24F ", 1)
...
titleStack.backgroundColor = new Color("#C41230", 1)
...
for (const item of res.groups[0].ranking){
    ...
    if(Math.abs(ranking - myTeamRanking) <= 2){
        ...
        if (ranking <= 3)
            clStack.backgroundColor = new Color("#C41230", 1)
        // 강등권 팀 설정
        else if (ranking >= 11)
            clStack.backgroundColor = new Color("#C41230", 1)
        ...
        // MyTeam 설정
        if(item.team.idInternal == myTeamId) 
            teamStack.backgroundColor = new Color("#C41230", 0.3)
        const underStack = widget.addStack()
        underStack.size = new Size(151, 1)
        underStack.borderColor = new Color("#C41230", 0.3)
...
function createStack(stack, text, width, isTitle) {
    ...
    elementText.textColor = new Color("#C41230", 1)
    if (isTitle == 1){
